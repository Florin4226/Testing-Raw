...
QBClothing.ResetItemTexture = function(obj, category) {
    ...
    $.post('https://vrp/updateSkin', JSON.stringify({
...
$(document).on('click', '.clothing-menu-option-item-right', function(e){
    ...
    if (canChange) {
        if (hasTracker && clothingCategory == "accessory") {
            $.post('https://vrp/TrackerError');
            return
        } else {
            if (clothingCategory == "model") {
                $(inputElem).val(newValue);
                $.post('https://vrp/setCurrentPed', JSON.stringify({ped: newValue}), function(model){
...
$(document).on('change', '.item-number', function(){
    ...
    if (hasTracker && clothingCategory == "accessory") {
        ...
        if (clothingCategory == "accessory" && inputVal == 13) {
            ...
            $.post('https://vrp/updateSkinOnInput', JSON.stringify({
                clothingType: clothingCategory,
...
$(document).on('click', '.clothing-menu-header-camera-btn', function(e){
    ...
    if (selectedCam == null) {
        ...
        $.post('https://vrp/setupCam', JSON.stringify({
            value: camValue
        }));
        selectedCam = this;
    } else {
...
$(document).on('keydown', function() {
    switch(event.keyCode) {
        case 68: // D
            $.post('https://vrp/rotateRight');
            break;
        case 65: // A
            $.post('https://vrp/rotateLeft');
            break;
        case 38: // UP
            ChangeUp();
            break;
...
$(document).on('click', "#save-menu", function(e){
    e.preventDefault();
    QBClothing.Close();
    $.post('https://vrp/saveClothing');
});

$(document).on('click', "#cancel-menu", function(e){
    e.preventDefault();
    QBClothing.Close();
    $.post('https://vrp/resetOutfit');
});
...
QBClothing.Open = function(data) {
   ...
   if(data.ilk_giris == 1){
    ...
    document.getElementById("co_face").style.display = "none";
    document.getElementById("co_model").style.display = "none";

   }else {

    document.getElementById("co_face").style.display = "block";
    document.getElementById("co_model").style.display = "block";
...
$(document).on('click', '.clothing-menu-outfit-option-button', function(e){
    ...
    $.post('https://vrp/selectOutfit', JSON.stringify({
...
$(document).on('click', '.clothing-menu-myOutfit-option-button-remove', function(e){
    ...
    $.post('https://vrp/removeOutfit', JSON.stringify({
        outfitData: outfitData.skin,
        outfitName: outfitData.outfitname,
        outfitId: outfitData.outfitId,
    }));
...
$(document).on('click', '#save-outfit-save', function(e){
    ...
    $.post('https://vrp/saveOutfit', JSON.stringify({
...
$(document).on('click', '.change-camera-button', function(e){
    ...
    $.post('https://vrp/rotateCam', JSON.stringify({
        type: rotationType
    }))
});
